...
public class ShopService {
    ...
    public boolean removeItem (Long orderId, Long productId){
        ...
        while (orderIterator.hasNext()){
            Order theLastOrder = orderIterator.next();
            for (int i = 0; i < theLastOrder.getItems().size() ; i++) {
                if(theLastOrder.getItems().get(i).getProductId().equals(productId)){
                    theLastOrder.getItems().remove(i);
                    return true;
                }
            }
        }
